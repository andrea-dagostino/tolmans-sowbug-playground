...
class TestRecorder:
    ...
    def test_save_csv(self):
        ...
        assert "reward_total" in rows[0]
        assert "reward_drive_reduction" in rows[0]
        assert "reward_shaping" in rows[0]
        assert "reward_urgency_penalty" in rows[0]
        assert "reward_off_target_penalty" in rows[0]
        assert "reward_urgent_explore_bonus" in rows[0]
        assert "reward_urgent_explore_penalty" in rows[0]
        assert "reward_stasis_penalty" in rows[0]
